...
def attention_decoder(mode, decoder_inputs, vocab_size, initial_state, encoder_states, enc_padding_mask, cell, initial_state_attention=False, pointer_gen=True, use_coverage=False, prev_coverage=None):
  """
  Args:
  ...
  with variable_scope.variable_scope("attention_decoder") as scope:
    ...
    for i, inp in enumerate(decoder_inputs):
      ...
      with variable_scope.variable_scope("AttnOutputProjection"):
        ...
        prob = tf.layers.dense(output, vocab_size)
    ...
    return outputs, probs, state, attn_dists, p_gens, coverage


def attention_generator(embedding, max_dec_steps, initial_state, encoder_states, enc_padding_mask, cell, initial_state_attention=False, pointer_gen=True, use_coverage=False, prev_coverage=None):
  ...
  with variable_scope.variable_scope("attention_decoder") as scope:
    ...
    embedding_size = embedding.get_shape()[1].value
    # Reshape encoder_states (need to insert a dim)
    encoder_states = tf.expand_dims(encoder_states, axis=2)  # now is shape (batch_size, attn_len, 1, attn_size)
    attention_vec_size = attn_size
    ...
    def attention(decoder_state, coverage=None):
      ...
      with variable_scope.variable_scope("Attention"):
        ...
        if use_coverage and coverage is not None:  # non-first step of coverage
          # Multiply coverage vector by w_c to get coverage_features.
          coverage = tf.cast(coverage, tf.float32)
    ...
    outputs = []
    base_seqs, sample_seqs, sample_dists = [], [], []
    probs = []
    attn_dists = []
    p_gens = []
    state = initial_state
    ...
    if initial_state_attention:  # true in decode mode
      # Re-calculate the context vector from the previous step so that we can pass it through a linear layer with this step's input to get a modified version of the input
      context_vector, _, coverage = attention(initial_state, coverage)  # in decode mode, this is what updates the coverage vector
    for i in range(0, max_dec_steps):
      # Run the attention mechanism.
      if i == 0:  # always true in decode mode
          inp = tf.nn.embedding_lookup(embedding, tf.ones([batch_size], dtype=tf.int64) * data_utils.BOS_ID)
          # x = tf.layers.dense(tf.concat([inp, context_vector],-1), embedding_size)
          x = linear([inp] + [context_vector], embedding_size, True)
      ...
      with variable_scope.variable_scope("AttnOutputProjection"):
        output = linear([cell_output] + [context_vector], cell.output_size, True)
        prob = tf.nn.softmax(tf.layers.dense(output, vocab_size))
        sample_id = tf.squeeze(tf.multinomial(tf.log(prob), 1))
        sample_id = tf.cast(sample_id,tf.int32)
        max_probs_id = tf.argmax(prob, -1)
        indices = tf.stack((batch_nums, sample_id), axis=1)
        sample_dist = tf.gather_nd(prob, indices)  # shape=(B,)
      sample_seqs.append(sample_id)
      sample_dists.append(sample_dist)
      base_seqs.append(max_probs_id)
      outputs.append(output)
      probs.append(prob)

      inp = tf.nn.embedding_lookup(embedding, max_probs_id)
      variable_scope.get_variable_scope().reuse_variables()
      x = linear([inp] + [context_vector], embedding_size, True)
    ...
    if coverage is not None:
      coverage = array_ops.reshape(coverage, [batch_size, -1])
    # generate_words.append(tf.ones((batch_size,), dtype = tf.int64)* utils_ROC.PAD_ID)
    base_seqs = tf.transpose(base_seqs)
    sample_seqs = tf.transpose(sample_seqs)
    sample_dists = tf.transpose(sample_dists)
    return base_seqs, sample_seqs,sample_dists, outputs, probs, state, attn_dists, p_gens, coverage
